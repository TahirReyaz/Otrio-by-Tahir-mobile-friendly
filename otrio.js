...
var winners = new Array(3), bigRingArray= new Array(12), smallRingArray= new Array(12), dotArray= new Array(12);
var targetPiece, movingPiece, bp, rp, yp, gp, currentPname, currentPNum, enabledColor, enabledColor2;
var playerNumber, p1name= undefined, p2name= undefined, winColour, winColour2 = "a";
...
for(i=0; i<5; i++)
{
    for(j=0; j<5; j++)
    {
        ...
        ////////////////////////////Setting the drop property of the center blocks//////////
            ...
            {
                pieceHolder.onclick=drop;
                pieceHolder.className = "centerSpace";
            }
...
for(i=0; i<12; i++)
{
    ...
    if((bigRingArray[i].id[6] == enabledColor || bigRingArray[i].id[6] == enabledColor2) && bigRingArray[i].id[6] != winColour && bigRingArray[i].id[6] != winColour2)
        bigRingArray[i].onclick = dragStart;
    else 
        bigRingArray[i].onclick = null;
    // Small Rings
    if((smallRingArray[i].id[6] == enabledColor || smallRingArray[i].id[6] == enabledColor2) && smallRingArray[i].id[6] != winColour && smallRingArray[i].id[6] != winColour2)
        smallRingArray[i].onclick = dragStart;
    else 
        smallRingArray[i].onclick = null;
    // Dots
    if((dotArray[i].id[6] == enabledColor || dotArray[i].id[6] == enabledColor2) && dotArray[i].id[6] != winColour && dotArray[i].id[6] != winColour2)
        dotArray[i].onclick = dragStart;
    else 
        dotArray[i].onclick = null;
}
}
...
function drop(event)
{
    ...
    if(event.target.id[1]== ",")
    {
        for(i=0; i<event.target.childNodes.length; i++)
            if(event.target.childNodes[i].id[0] == movingPiece.id[0])
                permission = false;
        if(permission)
        {
            event.target.appendChild(movingPiece);
            event.target.className = "pieceHolder";    
            movingPiece.onclick = null;
        }
    }
    ...
    else if (event.target.id[1] == "P")
    {
        ...
        for(i=0; i<event.target.parentNode.childNodes.length; i++)
            if(event.target.parentNode.childNodes[i].id[0] == movingPiece.id[0])
                permission = false;
        // If permission is given then drop the drop piece
        if(permission)
        {
            event.target.parentNode.appendChild(movingPiece);
...
function winCheck()
{
    ...
    var targetParentBlock= targetPiece.parentNode, checkBlock;
    ...
    for(i=0; i<targetParentBlock.childNodes.length; i++)
        if(targetParentBlock.childNodes[i].id[6] == targetPiece.id[6])
            point++;
    if(point == 3)
        winDisplay();
    point= 0;
    ...
    for(i=1; i<4; i++)
    {
        ...
        for(j=0; j<checkBlock.childNodes.length; j++)
            if(checkBlock.childNodes[j].id[0] == targetPiece.id[0] && checkBlock.childNodes[j].id[6] == targetPiece.id[6])
                point++;
    }  
    if(point == 3)
        winDisplay();
    ...
    for(i=1; i<4; i++)
    {
        ...
        for(j=0; j<checkBlock.childNodes.length; j++)
        {
            if(checkBlock.childNodes[j].id[0] == targetPiece.id[0] && checkBlock.childNodes[j].id[6] == targetPiece.id[6])
                point++;
            else if(checkBlock.childNodes[j].id[0] == targetPiece.id[0] && checkBlock.childNodes[j].id[6] == targetPiece.id[6])
                point++;
        }
    }  
    if(point == 3)
    ...
    for(j=0; j<checkBlock.childNodes.length; j++)
    {
        if(checkBlock.childNodes[j].id[0] == "d" && checkBlock.childNodes[j].id[6] == targetPiece.id[6])
            point++;
        if(checkBlock.childNodes[j].id[0] == "b" && checkBlock.childNodes[j].id[6] == targetPiece.id[6])
            decPoint++;
    }
    checkBlock = document.getElementById(y + "," + 2);
    for(j=0; j<checkBlock.childNodes.length; j++)
        if(checkBlock.childNodes[j].id[0] == "s" && checkBlock.childNodes[j].id[6] == targetPiece.id[6])
        {
            point++;
            decPoint++;
        }   
...
function winDisplay()
{
    ...
    if(currentPNum == playerNumber)
        winColour = targetPiece.id[6];
    else if(currentPNum == (playerNumber - 1))
        winColour2 = targetPiece.id[6];
